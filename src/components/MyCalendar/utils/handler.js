...
// 根据周起始重排 Weeks 数组
export function resortWeeks(start) {
  ...
  return function (m) {
    for (const marker of m) {
      ...
      if (['holiday', 'corner', 'schedule'].includes(type)) {
        ...
        const _marker = Object.prototype.hasOwnProperty.call(markers,key)
          ? markers[key]
          : {};

        _marker[type ] = Object.prototype.hasOwnProperty.call(_marker,type)
          ? _marker[type ]
          : [];
...
// 生成某月日历
export function month(year, month, markers) {
...
// 日期所在月份第几周
export function getDayWeekIdxInMonth(date, days) {
